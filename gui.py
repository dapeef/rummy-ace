...
PLAYER_CARDS_Y = 3*CARD_HEIGHT + 7*MARGIN
WIN_WIDTH = CARD_WIDTH * (NUM_CARDS_PER_PLAYER+1) + MARGIN * (NUM_CARDS_PER_PLAYER+2) + 2*PLAYER_CARDS_X
WIN_HEIGHT = CARD_HEIGHT * (2*NUM_PLAYERS+1) + MARGIN * (2*NUM_PLAYERS+7)
...
def draw_card(surface:pygame.surface.Surface, x:int, y:int, width:int=CARD_WIDTH, height:int=CARD_HEIGHT, face_up:bool=True, selected:bool=False, card_text:str="") -> pygame.Rect:
    ...
    # Draw the text
    text_surface = font.render(card_text if face_up else "", True, BLACK)
...
def check_card_click(position:tuple, card_rects:dict[str, pygame.Rect], game:Game, state:GUIState) -> None:
...
def main() -> None:
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left mouse button
                    check_card_click(event.pos, hit_boxes, game, state)
        ...
        pygame.draw.rect(
            screen,
            TABLE_COLOUR,
            pygame.Rect(MARGIN, MARGIN, WIN_WIDTH - 2*MARGIN, CARD_HEIGHT * (NUM_PLAYERS+1) + 5*MARGIN),
        ...
        pygame.draw.rect(
            screen,
            GREEN,
            pygame.Rect(PLAYER_CARDS_X, PLAYER_CARDS_Y + (CARD_HEIGHT+MARGIN)*game.whose_go, WIN_WIDTH - 2*PLAYER_CARDS_X, CARD_HEIGHT + 2*MARGIN),
            border_radius=CARD_CORNER_RADIUS + MARGIN
        )
